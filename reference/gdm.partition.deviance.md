...
Partitions deviance explained from GDM into different user specified
components - most typically environment versus space.
...
- varSets:
  ...
  A list in which each element is a vector of variable names across
  which deviance partitioning is to be performed, excluding geographic
  distance (which is set by the partSpace argument). Variable names must
  match those used to build the site-pair table. See example.

- partSpace:

  Whether or not to perform the partitioning using geographic space.
  Default=TRUE.
...
varSet
#> $soil
#> [1] "awcA"       "phTotal"    "sandA"      "shcA"       "solumDepth"
#> 
#> $climate
#> [1] "bio5"  "bio6"  "bio15" "bio18" "bio19"
#> 
